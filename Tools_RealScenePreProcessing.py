...
def CombineCamViewPointCloud_HPR(pcdPathList, camPathList, useHPR : bool = True, saveDIR = 'view'):
    ...
    for i, pack in enumerate(zip(pcdPathList, camPathList, varList)):
        ...
        if (useHPR):
            ...
            _, sub_points_map = pcd.hidden_point_removal(camPos, 40000)
...
if (__name__ == '__main__'):
    PCD_PATH = 'office_4\\mesh_Aligned.ply'
    VIEW_DIR = 'office_4\\view'
    # GetCamViewPointCloud(PCD_PATH, VIEW_DIR)
    CombineCamViewPointCloud_HPR([os.path.join(VIEW_DIR, 'cropped_1.ply'), os.path.join(VIEW_DIR, 'cropped_2.ply')], 
                                  [os.path.join(VIEW_DIR, 'camPara1.pkl'), os.path.join(VIEW_DIR, 'camPara2.pkl')], saveDIR = VIEW_DIR)
