...
def GetDataFromCSV(DIR_PATH, OBBName = 'OBB.csv', VOTE_DIR = 'votes', SEED_DIR = 'seeds', OBBScale = 1.0):
    global labelType
    ...
    for obbParam in obbParams:
        ...
        extent = np.array([obbParam[3], obbParam[4], obbParam[5]]) * OBBScale
...
def ObjectCombination(seedList, voteList, OBBList, OBJList, labels, combineIds = [], newList = None, visualize = False):
    ...
    for idx in combineIds:
        newSeeds += seedList[idx]
        newVotes += voteList[idx]
        newObj += OBJList[idx]
